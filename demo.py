...
comic_classes = np.asarray(['__background__',  # always index 0
...
#   input_dir = args.load_dir + "/" + args.net + "/" + args.dataset
  ...
  elif args.dataset == 'comic' or args.dataset == 'watercolor':
    classes = comic_classes
  ...
  colors = [(225, 0, 0), (0, 225, 0), (0,0,225), (225, 225,0), (225, 0, 225), (0, 225, 225)]
  while (num_images >= 0):
      total_tic = time.time()
      if webcam_num == -1:
        num_images -= 1
...
#       im = im_in[:,:,::-1]
      ...
      if vis:
          im2show = np.copy(im)
      color_dict = {}
      color_dict['default'] = colors[0]
      cnt = 0
      
      fig = plt.figure(frameon=False)
      fig.set_size_inches(im.shape[1] / 400, im.shape[0] / 400)
...
#             if (cls_dets.cpu().numpy()[:, -1] > 0.5).sum():
#               if classes[j] in color_dict.keys():
#                 color = color_dict[classes[j]]
#               else:
#                 color_dict[classes[j]] = colors[cnt]
#                 color = color_dict[classes[j]]
#                 cnt += 1
#             else:
#               continue
            if vis:
              ...
              im2show = vis_detections(im2show, classes[j], cls_dets.cpu().numpy(), 0.8, None, ax)
...
#           cv2.imwrite(result_path, im2show)
          fig.savefig(result_path, dpi=400)
          plt.close('all')
      else:
